<template>
  <div>
    <h1 class="title main-title">회원가입</h1>
    <form @submit.prevent="handleSignup">
      <div>
        ...
        />
        <span class="err-msg" v-if="isEmailDuplicate"
          >이 이메일은 이미 사용 중입니다.</span
        >
        <span class="err-msg" v-if="isEmailInvalid"
          >정확한 이메일을 입력해 주세요.</span
        >
        <span class="err-msg" v-if="emailErrors.length">{{
          emailErrors[0]
        }}</span>
      </div>
      <div>
      ...
      <div>
        ...
        />
        <span class="err-msg" v-if="firstPwInput.length < 8"
          >비밀번호는 8자 이상이어야 합니다.</span
        >
        <span class="err-msg" v-if="isPwCommon"
          >안전한 비밀번호를 사용해 주세요.</span
        >
        <span class="err-msg" v-if="passwordErrors.length">{{
          passwordErrors[0]
        }}</span>
      </div>
      <div>
        ...
        <input
          ...
          class="user-input"
        />
        <span class="err-msg" v-if="firstPwInput !== secondPwInput"
